...
21. [Updated Final Recipe & Open Questions](#21-updated-final-recipe--open-questions)
22. [Instructions for Coding Agent — Scaling to Full IPA](#22-instructions-for-coding-agent--scaling-to-full-ipa)
...
## 21. Updated Final Recipe & Open Questions

### Current Best Configuration

Based on the full S1–S12 + D2–D7 + E-series experimental sweep:
...
| Best E_geo (H-gated) | **0.440 ± 0.014** | D7 (3 seeds) |
| Best E_geo (T-edge oracle) | **0.162 ± 0.001** | D7 (3 seeds) |
...
All failures from Phases 7–17 still apply. New validated failures from D3–E9:
...
1. **Scale dependence of gating:** The 0.440 ceiling may be specific to the 2-slide toy regime. With more training slides, the gate may learn more robust neighbor discrimination. This is the strongest hypothesis for improvement.

2. **Gate architecture:** The current gate is a 2-layer MLP on $[h_i, h_j, |h_i - h_j|, h_i \odot h_j]$. A deeper or attention-based gate, or one that conditions on node context, might discriminate better. Not tested.

3. **β ramp during training (E10):** The agent's final inference-gap experiment — gradually increase β during training so the model progressively relies more on the gate. Not yet tested.

4. **Seed variance:** D7 achieves std = 0.014 across 3 seeds. The agent's threshold was ≤0.05 over 5 seeds, which may be met but hasn't been verified at 5+ seeds.

5. **ST↔SC compatibility (E11):** Untested. Relevant for the full pipeline but not for toy-scale experiments on ST-only data.

---

## 22. Instructions for Coding Agent — Scaling to Full IPA

### What Changed Since Phase 17 (S-Series)

The D3–D7 + E-series experiments added critical information:
...
2. **D7 is the definitive baseline.** E_geo = 0.440 ± 0.014 (H-gate), 0.162 (T-edge). All previous baselines should be compared against D7.
...
4. **Gate drift during joint training is a known, unsolved issue** (E8c-e). The pretrained gate is better than the jointly trained one, but the model can't learn from a frozen gate. No fix found at toy scale.
5. **Scaling data is the most promising path** to close the gap. More training slides = more diverse gate supervision signal = potentially better gate generalization.
...
11. Run attention diagnostics (Recall@k, logit_std, entropy vs recycle)
...
| **D7 H-gated inference** | **0.440 ± 0.014** | — | **V1 shared D7** | **Current best** |
...
**Paradox persists:** IPA-R has the *best* attention metrics but the *worst* E_geo. Simple V1 works best at toy scale.
