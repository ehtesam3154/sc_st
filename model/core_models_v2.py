...
class AdvancedHierarchicalDiffusion(nn.Module):
    ...
    def train_graph_vae(self, epochs=800, lr=1e-3, warmup_epochs=320,  # 40% of epochs
        ...
        for epoch in range(epochs):
            ...
            # Add latent smoothness over embedding-graph edges
            # i, j: (E,)
            i, j = adj_idx[0], adj_idx[1]

            # differences for all edges at once: (E, d)
            diff = z_st[i] - z_st[j]

            # squared L2 per edge: (E,)
            edge_loss = (diff * diff).sum(dim=1)

            # optional: weight by edge weights adj_w of shape (E,)
            if adj_w is not None:
                edge_loss = edge_loss * adj_w

            # reduction that's stable across different E
            L_smooth = edge_loss.mean()  # or edge_loss.sum() / (adj_w.sum()+1e-8)

            lambda_smooth = 0.01  # Small weight
    ...
    #     with torch.no_grad():
    #         st_features_aligned = self.netE(self.st_gene_expr).float()
    #         st_adj_idx_full, st_adj_w_full = precompute_knn_edges(st_features_aligned.detach(), k=30, device=self.device)
    #         st_mu, st_logvar = self.graph_vae_encoder(st_features_aligned, st_adj_idx_full, st_adj_w_full)
            
    #         # Precompute full embedding kNN for structure loss (will subgraph per batch)
    #         st_embedding_knn_full = precompute_knn_edges(st_features_aligned.detach(), k=30, device=self.device)
    ...
    #         #     batch_edge_idx, _ = precompute_knn_edges(batch_h.detach(), k=30, device=self.device)
    #         # i, j = batch_edge_idx[0], batch_edge_idx[1]

    #         # ---- structure-aware loss (decode x0, match edge distances) ----
    #         with torch.no_grad():
    #             # Extract subgraph for this batch from precomputed full graph
    #             # st_embedding_knn_full should be (2, E) long tensor on device
    #             adj_full = st_embedding_knn_full  # (2, E) long
                
    #             # idx: (B,) long global node indices for this batch
    #             B = idx.size(0)
                
    #             # Build a mapping from global id -> local [0..B-1]; -1 means "not in batch"
    #             N = self.st_gene_expr.size(0)  # total ST nodes
    ...
    #             # Map full-edge endpoints to local indices; negatives mark "out of batch"
    #             src_loc = map_gl2loc[adj_full[0]]    # (E,)
    #             dst_loc = map_gl2loc[adj_full[1]]    # (E,)
                
    #             # Keep only edges with both ends in the batch
    #             valid = (src_loc >= 0) & (dst_loc >= 0)
    #             if valid.any():
    #                 batch_edge_idx = torch.stack([src_loc[valid], dst_loc[valid]], dim=0)  # (2, E_batch)
    #             else:
    #                 batch_edge_idx = torch.empty((2, 0), dtype=torch.long, device=self.device)
                
    #             # Now safely unpack i, j
    #             i = batch_edge_idx[0] if batch_edge_idx.shape[1] > 0 else torch.tensor([], dtype=torch.long, device=self.device)
    #             j = batch_edge_idx[1] if batch_edge_idx.shape[1] > 0 else torch.tensor([], dtype=torch.long, device=self.device)


    #         # i, j = batch_edge_idx[0], batch_edge_idx[1] if batch_edge_idx.shape[1] > 0 else (torch.tensor([], device=self.device), torch.tensor([], device=self.device))
    ...
    def train_diffusion_latent(
        ...
        # Precompute ST latents (μ, logσ²) and ST aligned features (condition)
        print("Computing fixed ST latents and embedding kNN...")
        with torch.no_grad():
            st_features_aligned = self.netE(self.st_gene_expr).float()
            
            # Build graph from embeddings, not coordinates (FIX #1)
            st_adj_idx_for_vae, st_adj_w_for_vae = precompute_knn_edges(
                st_features_aligned.detach(), k=30, device=self.device
            )
            st_mu, st_logvar = self.graph_vae_encoder(st_features_aligned, st_adj_idx_for_vae, st_adj_w_for_vae)
            
            # UNPACK the KNN result properly for structure loss
            st_adj_idx_full, st_adj_w_full = precompute_knn_edges(
                st_features_aligned.detach(), k=30, device=self.device
            )
            
            # Make sure indices are long, shape is (2,E)
            adj_full = st_adj_idx_full
            if adj_full.dim() != 2:
                raise RuntimeError("adj_full must be (2, E)")
            if adj_full.shape[0] != 2:
                adj_full = adj_full.t()  # now (2, E)
            adj_full = adj_full.to(self.device).long()
            
            # Store weights if available
            w_full = st_adj_w_full.to(self.device) if st_adj_w_full is not None else None
            N = st_features_aligned.size(0)
        ...
        for epoch in range(n_epochs):
            ...
            B = min(self.batch_size, st_mu.shape[0])
            idx = torch.randperm(st_mu.shape[0], device=self.device)[:B]
            idx = idx.to(torch.long)  # Ensure long dtype
            ...
            # ---- structure-aware loss (decode x0, match edge distances) ----
            with torch.no_grad():
                # Map global -> local [0..B-1], -1 for "not in batch"
                map_gl2loc = torch.full((N,), -1, dtype=torch.long, device=self.device)
                map_gl2loc[idx] = torch.arange(B, device=self.device, dtype=torch.long)
                
                # Subgraph: pick edges whose both endpoints are in the batch
                src_glob = adj_full[0]  # (E,)
                dst_glob = adj_full[1]  # (E,)
                
                # Optional: guard against invalid indices
                valid_ids = (src_glob >= 0) & (src_glob < N) & (dst_glob >= 0) & (dst_glob < N)
                
                src_loc = map_gl2loc[src_glob]
                dst_loc = map_gl2loc[dst_glob]
                
                valid = valid_ids & (src_loc >= 0) & (dst_loc >= 0)
                if valid.any():
            ...
            if batch_edge_idx.shape[1] > 0:
                ...
                i = torch.tensor([], dtype=torch.long, device=self.device)
                j = torch.tensor([], dtype=torch.long, device=self.device)
            ...
            # Edge distances (only if we have edges)
            if i.numel() > 0:
                dist_true = (coords_true_b[i] - coords_true_b[j]).pow(2).sum(1).sqrt()
                dist_pred = (coords_pred[i] - coords_pred[j]).pow(2).sum(1).sqrt()
                L_struct = F.smooth_l1_loss(dist_pred, dist_true)
            ...
            # Warm up structure weight: 0 → 1.0
