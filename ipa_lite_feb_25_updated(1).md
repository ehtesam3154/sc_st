...
22. [Updated Final Recipe & Open Questions](#22-updated-final-recipe--open-questions)
23. [Instructions for Coding Agent — Scaling to Full IPA](#23-instructions-for-coding-agent--scaling-to-full-ipa)
...
## 22. Updated Final Recipe & Open Questions

### Current Best Configuration

Based on the full S1–S12 + D2–D7 + E-series + P-series experimental sweep:
...
| Best E_geo (H-gated) | **0.381 ± 0.004** | P2 B_aug_gate (2 seeds) |
...
1. **Scale dependence of gating:** The 0.381 ceiling may be specific to the 2-slide toy regime. With more training slides, the gate may learn more robust neighbor discrimination. This is the strongest hypothesis for improvement.

2. **Gate-attention balance:** The gate term overwhelms QK attention ($\beta \cdot \text{std}(g) / \text{std}(\text{qk}) \approx 2.5$). Finding a better balance — perhaps through gate logit normalization or learned β — might allow the structure module to contribute more effectively. P4v3-VALIDATE showed that simple β rescaling doesn't help, but learned dynamic β or attention-level gating might.

3. **Oracle distillation (C2):** The agent's recommended next step. Train the model with oracle (T-edge) attention, then distill that knowledge into the gated model. Untested.

4. **Seed variance:** P2 B_aug_gate achieves std = 0.004 across 2 seeds. The agent's threshold was ≤0.05 over 5 seeds. Needs verification at 5+ seeds.

5. **ST↔SC compatibility:** Untested. Relevant for the full pipeline but not for toy-scale experiments on ST-only data.

---

## 23. Instructions for Coding Agent — Scaling to Full IPA

### What Changed Since Phase 17 (S-Series)

The D3–D7 + E-series + P-series experiments added critical information:
...
4. **Gate augmentation (P2) is the one effective fix.** Feed H_aug (same augmentation as model) to the gate during training. This reduces gate overfitting and improves E_geo from 0.440 → 0.381.
...
8. **Scaling data remains the most promising path.** More training slides = more diverse gate supervision = potentially better gate generalization.
...
8.  Joint training:
    ...
    - Gate loss λ=0.05, constant β=2.0
...
| **P2 B_aug_gate** | **0.381 ± 0.004** | — | **V1 shared D7 + gate aug** | **Current best (P-series)** |
...
**Paradox persists:** IPA-R has the *best* attention metrics but the *worst* E_geo. Simple V1 works best at toy scale. Post-P-series best: $E_{\text{geo}} = 0.381$ with gate augmentation (P2).
